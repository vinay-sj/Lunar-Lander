...
class Lunar:
    ...
    def calculateFeatures(self, observation, action):
        ...
        for i in range(8):
            for j in range(4):
                features[144+i * 4 + j] = observation[i]**2 if action == j else 0

        for i in range(8):
            for j in range(4):
                features[176+i * 4 + j] = observation[i]**3 if action == j else 0

        for i in range(8):
            for j in range(4):
                features[208+i * 4 + j] = observation[i]**4 if action == j else 0
