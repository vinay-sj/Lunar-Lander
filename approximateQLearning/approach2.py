...
class Lunar:
    ...
    def calculateFeatures(self, observation, action):
        ...
        for i in range(8):
            for j in range(4):
                features[i * 4 + j] = self.sigmoid(observation[i]) if action == j else 0

        for i in range(7):
            for j in range(4):
                features[32 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 1]) if action == j else 0

        for i in range(6):
            for j in range(4):
                features[60 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 2]) if action == j else 0

        for i in range(5):
            for j in range(4):
                features[84 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 3]) if action == j else 0

        for i in range(4):
            for j in range(4):
                features[104 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 4]) if action == j else 0

        for i in range(3):
            for j in range(4):
                features[120 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 5]) if action == j else 0

        for i in range(2):
            for j in range(4):
                features[132 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 6]) if action == j else 0

        for i in range(1):
            for j in range(4):
                features[140 + i * 4 + j] = self.sigmoid(observation[i] * observation[i + 7]) if action == j else 0
